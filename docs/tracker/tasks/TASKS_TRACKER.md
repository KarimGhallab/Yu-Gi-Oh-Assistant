...
Ticket 19 is the frontier: the shared filter vocabulary (17) and the ranked
semantic lane (18) are resolved, so 19 can add the structured pre-filters that
join them. Spec 04 (Retrieval engine) is nearly ticketed out; specs 05
(Conversation store and CRUD) and 06 (Prompt-to-filter parsing) are unblocked and
