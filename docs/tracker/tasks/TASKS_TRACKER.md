...
tests) is ticketed as 45 through 51: 45 through 50 are resolved, so the fake
Ollama is a public surface, the stack stands up under the suite, a request
answers end to end, the readout shows the parse and edits it, the language and
model persist through a reopen, and a broken turn and a missing index say so. 51
is last: it puts the suite in CI behind the pipeline gate. Ticket 11 supersedes
