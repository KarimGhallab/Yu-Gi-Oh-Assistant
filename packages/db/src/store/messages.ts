...
export class MessageRepository implements IMessageRepository {
  ...
  async list(conversationId: number): Promise<Message[]> {
    const rows = this._database
      .prepare(
        `SELECT ${COLUMNS} FROM messages WHERE conversation_id = ? ORDER BY id ASC`
      )
      .all(conversationId);
...
function insert(
  ...
  const result = database
    .prepare(
      `INSERT INTO messages (conversation_id, role, content, filters_json, card_ids_json, created_at)
       VALUES (?, ?, ?, ?, ?, ?)`
    )
    .run(
      input.conversationId,
...
function toMessage(row: Record<string, unknown>): Message {
  return {
    ...
    conversationId: toNumber(row.conversation_id, 'conversation_id'),
